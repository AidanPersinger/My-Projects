...
# Loading `datascience`, `numpy`, and `plots`.
...
bgd_1970_through_2010 = b_five.exclude(10)
bgd_1975_through_2020 = b_five.exclude(0)
...
#Fill in code here
bangladesh = life_expectancy.where('geo', are.equal_to('bgd')).where('time', are.above_or_equal_to(1970))
bangladesh = bangladesh.plot('time', 'life_expectancy_years')
...
# No, the trends of life expectancy in the graph above are generally increasing over time (aside from a few slights dips) so it wouldn't explain why the population growth rate decreased from 1985 to 2015. A graph purely showing life expectancy with time is not sufficient information, because with the 1991 cyclone the mortality conditions would have changed for the people involved. Life expectancy at birth can only measure life expectancy given the mortality conditions remain the same throughout an individual's lifetime as to what they were when they were born, as stated before the beginning of q1_3.
...
def fertility_over_time(country_code, start):
    ...
    cleaned_table = country_fertility_after_start.drop('geo').relabel('children_per_woman_total_fertility', 'Children per woman').relabel('time', 'Year')
...
# Yes, you can see a decrease over time in fertility rates, and a decrease in fertility rates between 1980-2020 would in turn lead to a decrease in population growth, since there would be less children born each year and the population would therefore grow at a slower rate. 
...
post_1969_fertility_and_child_mortality.relabel('children_per_woman_total_fertility', 'Children per woman').relabel('child_mortality_under_5_per_1000_born', 'Child deaths per 1000 born')
...
# There seems to be a positive association between the number of children per woman and the number of child deaths per 1000 born. However, I would not say that is sufficient evidence to conclude that reduced child mortality is what caused parents to have fewer children, to prove causation we woud have to rule out other reasons parents might choose to have fewer children, such as lack of resources for example. 
...
#Fill in code here
population.where('time', are.between_or_equal_to(1800,2020)).drop('geo').group('time', np.sum).plot('time', 'population_total sum')
...
scatter_statements = make_array(1, 3)
...
recent_poverty_total = poverty_and_pop.with_column('poverty_total', np.round(poverty_and_pop.column('population_total') * (poverty_and_pop.column('poverty_percent') / 100))).drop('time')
...
poverty_percent = (np.sum(recent_poverty_total.column(3))/(population.drop(0).where('time', are.equal_to(2010)).group('time', np.sum).column(1).item(0)))*100
poverty_percent
...
def poverty_timeline(country):
    ...
    people_in_poverty = poverty_and_population.column('population_total') * poverty_and_population.column('extreme_poverty_percent_people_below_125_a_day')/100
    poverty_table = poverty_and_population.with_column('People In Poverty', people_in_poverty).plot('time', 'People In Poverty')
    # Don't change anything below this line. 
    plots.title(country)
    plots.ylim(bottom=0)
